...
The live configuration is intentionally conservative: Gemini 3.1 Flash Lite is primary,
all Gemini aliases share the project-scoped 14-RPM operational pacing bucket, alternate
keys are tried for key-specific authentication/restriction failures, and NVIDIA is last.
If every network entry fails, the event detail identifies each provider/model failure and
the next move is generated by the deterministic engine. A valid agent may still choose to
reject a commercially unattractive offer; fallback guarantees continuity, not a fake deal.
